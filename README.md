[![soifgo](https://github.com/soifgo/soifgo/blob/soifgo/soifgo-logo.png)](https://soifgo.github.io/soifgo/)



# 👋 Welcome to SoifGo
- [Soifgo](https://soifgo.github.io/soifgo/)

### The developer : Saeid Moghadam
(https://soifgo.github.io/soifgo/)
## 📜 License
This project is licensed under the MIT License - see the [LICENSE.md](LICENSE.md) file for details.




---

## 🌍 Supported Languages
- 🇬🇧 English (EN)  
- 🇪🇸 Spanish (Español)  
- 🇨🇳 Chinese (简体中文)  
- 🇫🇷 French (Français)  
- 🇩🇪 German (Deutsch)  
- 🇯🇵 Japanese (日本語)  
- 🇷🇺 Russian (Русский)  
- 🇸🇦 Arabic (العربية)  
- 🇮🇷 Persian (فارسی)  
- 🇮🇹 Italian (Italiano)  
- 🇮🇳 Hindi (हिन्दी)  
- 🇹🇷 Turkish (Türkçe)  
- 🇰🇷 Korean (한국어)  
- 🇵🇹 Portuguese (Português)  
- 🇮🇱 Hebrew (עברית)  
- 🇰🇼 Kurdish (کوردی)  
- 🇵🇰 Urdu (اردو)  
- 🇧🇩 Bengali (বাংলা)  
- 🇮🇩 Indonesian (Bahasa Indonesia)  
- 🇮🇳 Tamil (தமிழ்)  
- 🇹🇭 Thai (ไทย)  
- 🌍 Swahili (Kiswahili)  

## 🤝 Contributing
At this stage, the project is in its early phase.  
We welcome contributions mainly in the form of **translation improvements** and **language corrections**.  
If you notice any mistakes or awkward phrasing in your language, please open an issue and report it.  
Pull requests for translation fixes are also welcome.


## 📱 About SoifGo
SoifGo is an Android application that provides users with advanced capabilities using **buttons and seek bars**.  

### 🔧 Features
- 📡 **Bluetooth communication:** Sending and receiving serial Bluetooth data  
- 🌐 **API and MQTT server interaction & web browsing**
- 📝 **Managing HTML files & notes**  
- 📂 **Shortcut & library management for PDFs and images**  
- 🎛️ **Seek bar control with serial Bluetooth transmission**  
- 🎨 **Customizable buttons (size, color, shape, vibration, behavior)**  
- 🔄 **Dynamic effects based on Bluetooth input**  
- 📊 **Graph storage & review inside buttons**  
- 🎚️ **Predefined seek bar values for enhanced interaction**  
- 🌈 **Optimized RGB color control (WS2812, WS2813, WS2815, WS2811)**  
- ⚙️ **Bluetooth format selection (CR+LF, CR, LF, NULL, …)**  
- 🔁 **Smart Bluetooth resend (Check-in)**
  

## 📖 Tutorials

###  Examples and Tools
- [Create Page](https://soifgo.github.io/soifgo/tutorials/server/page/Create_Page.html)
- [Page & Folder Navigation](https://soifgo.github.io/soifgo/tutorials/server/pagefolder/page_folder.html)
- [Working with HTML Files](https://soifgo.github.io/soifgo/tutorials/server/WorkingwithHTMLFiles/html01.html)
- [Effects](https://soifgo.github.io/soifgo/tutorials/server/Effect/effects.html)
- [RANGMANG](https://soifgo.github.io/soifgo/tutorials/server/rangmang/Rangmang.html)

### Bluetooth and Device Interaction
- [Bluetooth Example 1](https://soifgo.github.io/soifgo/tutorials/server/BlutoothExample1/blutooth_sample1.html)
- [Bluetooth Send/Receive](https://soifgo.github.io/soifgo/tutorials/server/BluetoothSendReceive/Bluetooth_Send_Receive.html)

### Servers and APIs
- [Local Server Setup (Fastify on PC)](https://soifgo.github.io/soifgo/tutorials/server/LocalServerSetup/Local-server.html)
- [ESP8266 Server (WiFiServer/WebServer)](https://soifgo.github.io/soifgo/tutorials/server/ESP32Server/ESP8266_wifi.html)
- [Fetching APIs (External Servers)](https://soifgo.github.io/soifgo/tutorials/server/FetchingAPIs/Api-fetching.html)

### MQTT
- [ESP8266 ](https://soifgo.github.io/soifgo/tutorials/server/Esp8266_wifi_mqtt/esp8266_mqtt.html)
- 
---

## 🚀 Installation
- Download the latest APK from [Releases](https://github.com/soifgo/soifgo/releases).
- Install on your Android device.
- Connect via Bluetooth and start customizing!


---

## 🔑 Keywords
soifgo, android-app, apk, custom-ui, programmable-buttons, custom-html, graph,  
bluetooth-serial, bluetooth-uart, hc-05, bluetooth-microcontrollers, rgb-program,  
api, server, swift-design ,soifdesign ,WS2812, WS2813, WS2815, WS2811 , rangmang ,
JDY-31 spp-c bluetooth module ,note ,Command Output ,Message Output ,Data Output ,
Note Output ,sensor-transmission ,sensor-bluetooth ,sensor-api ,mobile-sensor-data


